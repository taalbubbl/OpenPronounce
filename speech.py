...
def extract_embeddings(audio_waveform, sampling_rate=16000):
    ...
    inputs = feature_extractor(
        audio_waveform, sampling_rate=sampling_rate, return_tensors="pt", padding=True
    )
...
def compare_transcriptions(transcription, text_reference):
    ...
    transcribed_phonemes, transcribed_map = get_phonemes_with_word_mapping(
        transcription_clean
    )
            ...
            for tidx in sorted_trans_indices:
                if tidx in transcribed_map:
                    ...
                    if w not in seen_words:  # This prevents "I I" -> "I"
                        ...
                        seen_words.add(
                            w
                        )  # This prevents "that that" -> "that that" if they are identical?
    ...
    if words_with_errors:
        feedback += (
            "❌ You need to better pronounce these words: "
    ...
    expected_vector, transcribed_vector = align_sequences_dtw(
        expected_vector, transcribed_vector
    )
...
def compute_pronunciation_score(
    distance_dtw, phoneme_distance, word_distance, max_dtw=500, max_lev=30
):
    """
...
def compare_audio_with_text(audio_1, text_reference, sampling_rate=16000):
    ...
    score = compute_pronunciation_score(
        distance, differences["phoneme_distance"], differences["word_distance"]
    )
...
def transcribe(audio):
    ...
    return ctc_processor.batch_decode(predicted_ids)[0]
